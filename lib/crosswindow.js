var __crossWindow = new function() {
  ...
  __crossWindow.prototype.set = function(key, value, callback) {
    var callerFunc = ((new Error).stack || arguments.callee.toString()).split("\n")[1] || 'anonymous';
    var msgID = (callerFunc.substring(0, callerFunc.indexOf("@")).replace(/[<>]+/g, '') || "anoynmous") + '_' + String((new Date).getTime() + Math.random()).hashCode();
  ...
  __crossWindow.prototype.bind = function(type, callback, listenerID) {
    ...
    if(!listenerID) {
      var callerFunc = ((new Error).stack || arguments.callee.toString()).split("\n")[1] || 'anonymous';
      var listenerID = (callerFunc.substring(0, callerFunc.indexOf("@")).replace(/[<>]+/g, '') || "anoynmous") + '_' + String((new Date).getTime() + Math.random()).hashCode();
      var index = 0;
  ...
  __crossWindow.prototype.getListeners = function() {
